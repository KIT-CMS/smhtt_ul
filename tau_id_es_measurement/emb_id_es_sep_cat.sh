...
if [[ $MODE == "IMPACTS_ID" ]]; then
    ...
    combineTool.py -M Impacts  -d ${WORKSPACE_IMP} -m 123 \
        --setParameters ES_DM0=${imp_es_dm0},r_EMB_${id_var_imp}=${id_dm0} \
...
if [[ $MODE == "IMPACTS_ALL" ]]; then
    ...
    combineTool.py -M Impacts -d $WORKSPACE_IMP -m 123 -o tauid_${WP}_impacts_r_${dm_cat}_v4.json  

    plotImpacts.py -i tauid_${WP}_impacts_r_${dm_cat}_v4.json -o tauid_${WP}_impacts_r_${dm_cat}_v4
    # # cleanup the fit files
    rm higgsCombine_paramFit*.root
    # rm robustHesse_paramFit*.root
    mv tauid_${WP}_impacts_r_${dm_cat}_v4* $impact_path/${ERA}/${CHANNEL}/${WP}
    done
