...
# Adjust es_shifts4p0 to what ES variations your NTuples contain! Might got from 2.5 instead of 4.0. 
# Also check in [used scripts that have similar dicts]
es_shifts4_0=(
...
if [[ $MODE == "COPY" ]]; then
    ...
    echo "##############################################################################################"
    echo "#      Copy sample to ceph, if not there yet                                                 #"
    echo "##############################################################################################"
...
PROCESSES="emb"
number="_emb_ssos"
# For local processing, rarely used due to slow processing
if [[ $MODE == "LOCAL" ]]; then
    source utils/setup_root.sh
    python shapes/produce_shapes_tauid_es.py --channels $CHANNEL \
        --directory $NTUPLES \
        --${CHANNEL}-friend-directory $XSEC_FRIENDS \
        --era $ERA --num-processes 8 --num-threads 4 \
        --vs-jet-wp $WP \
        --vs-ele-wp ${VS_ELE_WP} \
...
if [[ $MODE == "SYNC" ]]; then
    ...
    # Syncing to combine naming convention:
    echo "##############################################################################################"
    echo "#     synced shapes                                                                          #"
    echo "##############################################################################################"
...
# Control plots for different ES:
if [[ $MODE == "PLOT-CONTROL-ES" ]]; then
    ...
    do
        ...
        # for es_sh in "${es_shifts4_0[@]}" !!! out for quick check!
        for es_sh in "emb0p0"
        do 
            python3 plotting/plot_shapes_control_es_shifts.py -l --era Run${ERA} --input ${shapes_rootfile} \
...
# Needed for next Modes! May exists already:
...
if [[ $MODE == "DATACARD_DM" ]]; then
    ...
    do
        # This inputfile exists but is not getting used further down the script ???
...
if [[ $MODE == "SCAN_2D" ]]; then
    ...
    do
        ...
        # combineTool.py -M FastScan -w output/$datacard_output_dm/htt_mt_${dm_cat}/ws_scan_${dm_cat}.root:w
...
# Optional if some nuis paras seem "shady" from MULTIFIT !!!:
if [[ $MODE == "PROBL_NUIS_SCAN" ]]; then
   ...
   echo "[INFO] Create 1D scan for problematic nuisance parameter ${probl_nuisance}"
    ...
    # This root file does not exist? higgsCombine.nominal_... ???
...
# Impact plots need seperated fit:
...
if [[ $MODE == "POSTFIT_MULT" ]]; then
    ...
    # Produce postfitshapes in readable form:
