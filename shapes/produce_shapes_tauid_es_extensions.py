...
def parse_arguments():
    ...
    parser.add_argument(
        "--vs-jet-wp", required=True, type=str, help="Tau ID Vs Jet WP."
    )
    parser.add_argument(
        "--vs-ele-wp", required=True, type=str, help="Vs Ele Fake rate WP."
    )
    parser.add_argument(
        "--vs-mu-wp", required=True, type=str, help="Vs Mu Fake rate WP."
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--binning-tag",
        default="WP_ERAshort_xtoz",
        type=str,
        help="Tag to find the correct binning of the shapes.",
    )
    parser.add_argument(
        "--selection-option",
...
def TauES_TauID_histogram_booking(
    ...
    elif channel == "mt" and args.special_analysis == "TauID":
        logger.info("Booking TauID")
        shape_utils.book_tauES_histograms(
            manager=unit_manager,
            additional_emb_procS=processes,
            datasets=datasets,
    ...
    elif channel == "mt" and args.special_analysis == "TauID_ES":
...
def main(args):
    ...
    for channel in args.channels:
        ...
        if args.special_analysis == "TauES":
            ...
            shape_utils.add_tauES_datasets(
                ...
                tauESvariations,
                [
                    channel_selection(channel, args.era, args.special_analysis, args.vs_jet_wp, args.vs_ele_wp, args.vs_mu_wp),
                    selection.ZTT_embedded(channel, args.era, args.apply_tauid, args.vs_jet_wp, args.vs_ele_wp, args.vs_mu_wp),
                ],
                categorization,
    ...
    for channel in args.channels:
        ...
        if channel == "mt" and args.special_analysis in ["TauID", "TauID_ES"]:
        ...
        if not args.skip_systematic_variations:
            ...
            if channel in ["et", "mt", "tt"]:
                if args.special_analysis not in ["TauID", "TauID_ES"]:
                    _book_histogram(
