...
def main(args):
    ...
    for key in input_file.GetListOfKeys():
        ...
        if category != "control_region" and not args.gof:
            if "EMB" in process:
                process  = "EMB"#+category
            if "emb" in process:
                process = "EMB_"+category
        ...
        if process == "jetFakes":  # Check for the incorrect pattern
            ...
            if "Down_" in name_output:
                ...
                if len(parts) == 2:
                    ...
                    logger.debug(f"Correcting jetFakes name: {name_output} -> {corrected_name}")
                    name_output = corrected_name
        if "_selection" in process and "_selection125" not in process:
            name_output = name_output.replace("_selection", "_selection125")
        logging.debug(
