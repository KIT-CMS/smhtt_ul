...
def parse_arguments():
    ...
    parser.add_argument(
        ...
        type=str,
        default="ff",
        help="Type of fake factor to use: 'raw_ff' or 'ff' ")
    parser.add_argument(
        "--embedding",
        action="store_true",
        help="Fake factor estimation method used")
...
def main(info):
    ...
    rare_proc = {"EWK", "VVVL", "VVVT", "VVVJ", "TTVL", "TTVT", "TTVJ", "ggH", "qqH", "ttH", "VH", "W"} & set(bkg_processes)
    ...
    save_path = args.output if args.output else os.path.join(f"outputs/ml/{args.era}_plots_{postfix}_{args.tag}", channel)
    if not os.path.exists(save_path):
        os.makedirs(save_path, exist_ok=True)
    for _ext in ["pdf", "png"]:
        plot.save(os.path.join(save_path, f"{args.era}_{channel}_{args.category or ''}_{variable}.{_ext}"))
...
if __name__ == "__main__":
    ...
    if not args.embedding and not args.fake_factor:
        ...
        b = f"{args.ff_type}" if args.fake_factor else "classic"
        postfix = f"{a}_{b}"
    if args.nlo:
