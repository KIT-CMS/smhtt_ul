...
def main(info):
    ...
    for i in range(2):
        ...
        for process in legend_bkg_processes:
            ...
            # print("STYLES LABEL", styles.legend_label_dict[process.replace("TTL", "TT").replace("VVL", "VV").replace("_NLO","")])
            if "mm" in channel:
                if process == "EMB":
                    plot.legend(i).add_entry(
                            0, process, "#mu#rightarrow#mu embedded", 'f')
                else:
                    plot.legend(i).add_entry(
                    0, process, styles.legend_label_dict[process.replace("TTL", "TT").replace("VVL", "VV").replace("_NLO","")], 'f')
            elif "mm" not in channel:
                plot.legend(i).add_entry(
                    0, process, styles.legend_label_dict[process.replace("TTL", "TT").replace("VVL", "VV").replace("_NLO","")], 'f')
