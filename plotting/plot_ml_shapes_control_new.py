...
STAGES = {
    ...
    "stage0": {
        "sig_processes": (
            "qqH125-vbf_htautau_bin201to210_selection",
            "ggH125-ggh_htautau_bin101to116_selection",
        ),
        "signal_categories": (
            (100, r"VBF", (0.5, 2.0)),
            (101, r"ggH", (0.5, 1.5)),
    ...
    "stage0_HIG-19-010": {
        ...
        "signal_categories": (
            (1, r"ggH incl. $\times$ VBF incl.", (0.5, 2.0)),
        ),
    },
}
...
def main(args):
    ...
    if args.is_postfit:
        input_root = ntuple_dir / "datacards" / args.channel / "125" / "shapes" / "datacard-shapes-postfit.root"
        shapes_root = ntuple_dir / "datacards" / args.channel / "125" / "shapes" / "datacard-shapes-postfit.root"
    else:
        input_root = ntuple_dir / f"datacards{'.for_plotting' if args.mask_signal_region else ''}" / args.channel / "125" / "shapes" / "datacard-shapes-prefit.root"
        shapes_root = ntuple_dir / f"datacards{'.for_plotting' if args.mask_signal_region else ''}" / args.channel / "125" / "shapes" / "datacard-shapes-prefit.root"

    if args.save_path:
        save_path = args.save_path
    else:
        save_path = ntuple_dir / f"datacards{'.for_plotting' if args.mask_signal_region else ''}" / args.channel / "125" / "shapes" / "plots"
    ...
    if args.is_HIG_19_010:
        stage = STAGES["stage0_HIG-19-010"]
        global BKG_CATEGORIES
        BKG_CATEGORIES = BKG_CATEGORIES_HIG_19_010

    category = lambda cat_id: f"htt_{args.channel}_{cat_id}_{args.era}_{'postfit' if args.is_postfit else 'prefit'}"
    ...
    plot_nn_output_from_combine_shapes_file(
        ...
        show=False,
        category_data_masking_condition=None if args.mask_signal_region else lambda cat: False,  # default is masked signal
        total_uncertainty_type="postfit" if args.is_postfit else "prefit",
    )
