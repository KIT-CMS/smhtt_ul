...
if [[ "$SUBMIT_MODE" == "multigraph" ]]; then
    ...
    echo "[INFO] Using friends $FF_FRIENDS $RECO_FRIENDS $NN_FRIENDS"
    [[ ! -d $OUTPUT ]] && mkdir -p $OUTPUT

    if [[ $BOOSTED == 0 ]]; then
        python shapes/produce_shapes.py --channels $CHANNEL \
            --directory $NTUPLES \
            --${CHANNEL}-friend-directory $XSEC_FRIENDS $FF_FRIENDS $NN_RES_FRIENDS \
            --era $ERA --xrootd \
        ...
        python shapes/produce_shapes.py --channels $CHANNEL \
            --directory $NTUPLES \
            --${CHANNEL}-friend-directory $XSEC_FRIENDS $FF_FRIENDS $NN_BOOST_FRIENDS \
            --era $ERA --xrootd \
            --optimization-level 1 \
            --only-create-graphs \
            --graph-dir $OUTPUT \
        ...
        # Same for boosted tautau submission
